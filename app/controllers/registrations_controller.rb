  ...
  def create
    ...
    if !::Milia.use_recaptcha  ||  verify_recaptcha
      ...
      Tenant.transaction  do
        ...
        if @tenant.errors.empty?   # tenant created
          if @tenant.plan == 'premium'
            @payment = Payment.new({ email: user_params["email"],
                                     ...
                                     tenant: @tenant })
            flash[:errors] = "Please check registration errors" unless @payment.valid?
        ...
        if @tenant.plan == 'free' || flash[:errors].blank? || flash[:errors].empty? #payment successful
      ...
      flash[:errors] = "Recaptcha codes didn't match; please try again"
